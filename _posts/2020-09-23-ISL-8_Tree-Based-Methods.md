...
재귀 이진 분할을 수행하기 위해서는 먼저 예측변인 $$X_j$$와 예측변인 공간을 $$\left\{X|X_j < s\right\}$$와 $$\left\{X|X_j ≥ s\right\}$$로 나눔으로써 RSS를 가장 크게 줄이는 컷포인트 $$s$$를 선택한다. 즉, 모든 예측변인 $$X_1, . . .,X_p$$와 각 예측변인에 대한 컷포인트 $$s$$의 모든 가능한 값들을 고려하여 RSS가 가장 낮은 트리를 만드는 예측변인과 컷포인트를 고르는 것이다. 수식으로 표현하면 어떤 $$j$$와 $$s$$에 대한 반평면(half-planes)의 짝을 아래와 같이 정의할 수 있다.


$$
R_1(j, s) = \left\{X|X_j < s\right\}, \quad R_2(j, s) = \left\{X|X_j ≥ s\right\}
$$
