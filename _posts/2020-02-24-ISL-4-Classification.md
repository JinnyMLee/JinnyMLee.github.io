...
로지스틱 회귀가 예측변인 $$X$$가 주어졌을 때 반응변인 $$Y$$에 대한 조건부 분포를 모델링하는 반면, **선형 판별 분석(Linear Discriminant Analysis; LDA)**은 $$Y$$가 주어졌을 때 각 범주에서의 예측변인 $$X$$의 분포를 모델링하고, 베이즈 정리를 통해 이것을 $$Pr(Y = k\vert X = x)$$에 대한 추정치로 전환한다.
